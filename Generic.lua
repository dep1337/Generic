...
local width, height = 256, 384 -- Width and height of parent frame
local insets = { left = 20,	right = -20, top = -20, bottom = 20 } -- insets.top also needs to include the title container (see Section 2)
local usableW, usableH = width + insets.right - insets.left, height + insets.top - insets.bottom -- Width and height excluding insets
...
local function dumpMethods(object)
	...
	for k, v in pairs(meta) do
		if (type(v) == "function") then print("function ", k, "()") end
	end
end

-- Debugging function to print the attributes of an object
local function dumpAttributes(object, lvl)
	if (type(lvl) ~= "number") then lvl = 1 end -- If no level is provided (ie. omitted or not a number) then set it to level 1
	for k, v in pairs(object) do
		if (type(v) ~= "function") then print(strrep(".", lvl), k, " = ", v) end
		if (type(v) == "table") then dumpAttributes(v, lvl + 1) end
	end
end

-----------------------------------------------------------------------------
-- SECTION 2: Create the parent frame and implement core functionality including:
-- 1. Properly centred gtitle
...
--dumpTable(frame)
--print(dumpTable(GetTextureInfo(frame.Bg)))
--dumpTable(frame.Bg)
